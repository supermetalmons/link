...
test("uses wagers-only socket protocol and keeps validated JWTs out of URLs", () => {
  assert.deepEqual(
    createInviteWagersSocketProtocols("header.payload.signature"),
    ["mons-invite-wagers-v1", "bearer.header.payload.signature"],
  );
  assert.equal(
    getInviteWagersSocketUrl("invite"),
  ...
  for (const token of [
    ...
    assert.throws(() => createInviteWagersSocketProtocols(token), {
      code: "invalid-wagers-socket-token",
    });
  }
  assert.throws(() => getInviteWagersSocketUrl("bad/invite"), {
    code: "invalid-invite",
  });
});
...
test("reads a bounded authenticated snapshot and permits public spectator reads", async () => {
  ...
  assert.deepEqual(
    await readInviteWagersViaApi("invite", async () => "token"),
    value(),
  );
  await readInviteWagersViaApi("invite");
  assert.equal(calls[0].url, "https://api.mons.link/invites/invite/wagers");
...
test("refreshes auth exactly once for 401 and preserves the shared deadline", async () => {
  ...
  await readInviteWagersViaApi("invite", async (refresh) => {
  ...
  await assert.rejects(
    readInviteWagersViaApi("invite", async () => "token"),
...
test("keeps missing, forbidden, unavailable and rate-limit errors distinct and honors Retry-After", async (t) => {
  ...
  await assert.rejects(readInviteWagersViaApi("invite"), {
    retryAfterMs: 60_000,
  });
  globalThis.fetch = async () =>
...
test("rejects malformed, foreign or credential-bearing responses", async () => {
  for (const payload of [
    ...
    value({ wagers: { invite: { settlementClaim: "private" } } }),
    { ...value(), viewer: { actorUid: "host" } },
    { ...value(), wagers: {} },
  ]) {
...
test("bounds declared and streamed payloads and cancels oversized bodies", async () => {
  ...
  for (const headers of [
    { "Content-Length": String(INVITE_WAGERS_MAX_MESSAGE_BYTES + 1) },
    {},
  ]) {
    globalThis.fetch = async () =>
      new Response(
        new ReadableStream({
          start(controller) {
            controller.enqueue(
              new Uint8Array(INVITE_WAGERS_MAX_MESSAGE_BYTES + 1),
            );
          },
          cancel() {
            canceled++;
...
test("aborts pending token lookup without a late request", async () => {
  ...
  const pending = readInviteWagersViaApi("invite", () => token.promise, {
...
test("one deadline bounds token refresh and rejects late tokens even before delayed timer callbacks", async (t) => {
  ...
  const token = deferred();
  globalThis.fetch = async () => response({}, 401);
  const pending = readInviteWagersViaApi("invite", (refresh) =>
    refresh ? token.promise : Promise.resolve("old-token"),
  );
  const rejected = assert.rejects(pending, { code: "timeout" });
  await flush();
  t.mock.timers.tick(INVITE_WAGERS_REQUEST_TIMEOUT_MS);
  await rejected;
  ...
  const late = deferred();
  const delayed = readInviteWagersViaApi("invite", () => late.promise);
  now += INVITE_WAGERS_REQUEST_TIMEOUT_MS;
...
test("aborts stalled response streams as well as the request", async () => {
  ...
  const pending = readInviteWagersViaApi("invite", undefined, {
    signal: controller.signal,
  });
  await flush();
  controller.abort();
...
test("rejects auth-user replacement and cancels late response bodies", async () => {
  ...
  const pending = readInviteWagersViaApi("invite", tokenProvider);
  await flush();
  current = false;
  request.resolve(
    new Response(
