...
Account-link game discovery uses only the activated `PROFILE_GAMES_DB.login_match_discovery` index, with no Firebase discovery fallback. It covers login UIDs independently of profile ownership, including games played anonymously before account linking. The migration operator, `npm run manage:login-match-discovery`, streams protected Firebase key inventories and verifies complete D1 coverage while gameplay and Queues stay active. See [the completed discovery cutover](scripts/deploy-cloudflare.md#account-link-game-discovery-cutover). Invite projection still reads retained RTDB gameplay state.
