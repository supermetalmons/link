...
const createEventLockManager = (dependencies = {}) => {
  ...
  const createLockId =
    dependencies.createLockId || (() => randomAlphanumeric(16));
  ...
  const acquireEventLock = async (eventId, ownerUid) => {
    const ref = getDatabase().ref(`${lockRoot}/${eventId}`);
    ...
    return { eventId, ref, lockId, ownerUid, lockRoot };
  };
  ...
  const refreshEventLock = async (lockHandle) => {
    ...
    const result = await runTransaction(lockHandle.ref, (current) => {
