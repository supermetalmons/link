...
test("copies each source game and deletes it in the same commit", async () => {
  ...
  const job = {
    ...
    sourcePhase: "games" as const,
  };
  const source = document("users", "source-profile", {
    logins: [],
    mergedIntoProfileId: "target-profile",
  });
  const target = document("users", "target-profile", { logins: [] });
  const mapping = document("profileMergeTargets", "source-profile", {
  ...
  const gameCommit = memory.commitBatches.find((writes) =>
    writes.some((write) => "delete" in write && write.delete === gameName),
  );
  assert.ok(gameCommit);
  assert.equal(
    gameCommit.some(
      (write) =>
        "update" in write &&
  ...
  assert.equal(
    memory.documents.has(authRecoveryJobName("target-profile")),
    false,
  );
});

test("drains a game written after the source parent is deleted", async () => {
  let nowMs = 1_000;
  const job = {
    ...newAuthRecoveryJob("target-profile", [], ["source-profile"], 0),
  ...
  const memory = memoryFirestore([
    ...
    document("profileMergeTargets", "source-profile", {
      targetProfileId: "target-profile",
    }),
  ]);
  const recovery = createAuthRecoveryService(TELEGRAM_TEST_ENV as Env, {
  ...
  assert.equal(await recovery.recoverProfile("target-profile"), false);
  assert.equal(memory.documents.has(lateGameName), false);
  assert.equal(
