...
Firebase custom `profileId` claims and RTDB `players/{uid}/profile` links are non-authoritative compatibility shadows. They remain for retained Firebase read rules, claim and link recovery, and missing-profile cleanup; live move authorization uses canonical D1 ownership in the Worker. Worker authorization and canonical projection ownership never fall back to either shadow.
