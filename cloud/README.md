...
Firebase custom `profileId` claims are retired from the browser and Worker runtime: runtime code never reads, writes, or deletes them, and existing stored claims remain untouched. Firebase Auth still provides anonymous sessions and identity tokens. RTDB `players/{uid}/profile` links are retired: runtime code never reads, writes, or deletes them. Client reads and writes to these links and all retained `invites/{inviteId}` data are denied, including clients with an admin claim. Retain existing records as historical evidence. Worker authorization and canonical projection ownership never fall back to claims or retained links.
