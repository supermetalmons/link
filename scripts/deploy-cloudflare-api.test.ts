...
test("smokes NFT, authenticated, and X callback routes with bounded retries", async () => {
  ...
  const authRequests = requests.slice(4, 32);
  assert.deepEqual(
    ...
    [
      ...
      [`${PREVIEW_URL}/matches/timer/start`, "OPTIONS"],
      [`${PREVIEW_URL}/matches/timer/start`, "POST"],
      ...
      [`${PREVIEW_URL}/wagers/proposals/cancel`, "POST"],
      [`${PREVIEW_URL}/wagers/proposals/decline`, "OPTIONS"],
      [`${PREVIEW_URL}/wagers/proposals/decline`, "POST"],
      [`${PREVIEW_URL}/wagers/proposals/send`, "OPTIONS"],
      [`${PREVIEW_URL}/wagers/proposals/send`, "POST"],
    ],
  );
  assert.equal(
    requests[35].url,
  ...
  assert.equal(requests[32].url, `${PREVIEW_URL}/internal/telegram/delivery`);
  assert.equal(requests[32].init.method, "POST");
  assert.equal(
    requests[33].url,
    `${PREVIEW_URL}/internal/telegram/event-prize-announcement`,
  );
  assert.equal(requests[33].init.method, "POST");
  assert.equal(requests[34].url, `${PREVIEW_URL}/auth/x/callback`);
  assert.equal(requests[34].init.method, "GET");
  assert.equal(requests[35].init.method, "GET");
...
test("production promotes only the explicit version and then smokes the custom domain", async () => {
  ...
  const dependencies = createRuntimeDependencies({
    ...
    fetch: async (url) => {
      assert.match(
        String(url),
        /^https:\/\/api\.mons\.link\/(?:nfts|mining\/rock|automatch\/(?:cancel|start)|leaderboards\/read|matches\/timer\/start|navigation\/games\/remove|profiles\/lookup|wagers\/proposals\/(?:accept|cancel|decline|send)|internal\/telegram\/(?:delivery|event-prize-announcement)|auth\/(?:intents|methods|x\/(?:flows|callback)))/,
      );
      return nextResponse(responses);
    },
    log: (message: string) => logs.push(message),
