...
test("smokes NFT, authenticated, and X callback routes with bounded retries", async () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      [`${PREVIEW_URL}/ratings/update`, "OPTIONS"],
      [`${PREVIEW_URL}/ratings/update`, "POST"],
...
test("preview validates, uploads with strict mode, sanitizes secrets, and smokes", async () => {
  ...
  assert.equal(calls[2].environment.RATING_SERVICE_ACCOUNT_EMAIL, undefined);
  assert.equal(
...
test("trigger updates use only the selected token in a sanitized environment", async () => {
  ...
  assert.equal(calls[0].environment.RATING_SERVICE_ACCOUNT_EMAIL, undefined);
  assert.equal(
    calls[0].environment.RATING_SERVICE_ACCOUNT_PRIVATE_KEY,
    undefined,
  );
