...
## Firebase retirement record

The active-match migration completed September 11, 2026. Its immutable D1 evidence records 9,821 imported matches, 9 timer claims, and 4,629 rooms; source and verification digest `486123eba6e315ccd0d0c911bdcd706a3f823ccfb03a8217d6db15aa70aec6aa` remains unchanged. Current counts can grow through gameplay.

Released September 11, 2026 with API version `b429c595-0985-45a9-932e-329e7d5ca1d0` and frontend version `d2466b28-3f49-4897-9044-d5571144e2b4`, each serving 100% of traffic. Event-progress Workflow definition `c2ad0435-3c58-4131-b863-a914708ecf7a` and withdrawal definition `066ea8c3-507d-4c43-a69c-35995d1f7162` were published while preserving existing instances and settings. Four unused provider service-account bindings and both completed migration secrets were removed. Other bindings and the existing Durable Object namespace are unchanged.

The complete gate passed 3,066 tests: 664 client, 958 API Node, 602 Worker runtime, 308 tooling, and 534 portable-runtime tests. Formatting, lint, typechecks, generated bindings, and the API dry-run passed. Production verification passed API smoke, all 16 isolated gameplay checks, the dedicated wager lifecycle, five linked-profile recovery/replay checks, nine real-browser checks, retained event/wager/history reads, and the withdrawal Workflow's read-only preflight. Five immutable snapshots matched across release and local cleanup. Test series were ended and temporary sessions revoked.

All 58 applied SQL migrations remain unchanged. Existing Cloudflare identities, records, and exact serialized replay formats are preserved. Forty-seven verified obsolete local artifacts were removed, including both remaining cutover archives and the retired browser API-key environment file. No local source archive was retained; current Cloudflare credentials and shared provider authentication were preserved.

Google project `mons-link` (`390871694056`), including `mons-attester`, has not been deleted. Automatic approval review rejected the project-wide deletion before execution because it classified the broad, potentially irreversible action as prohibited even with explicit approval. The project was verified `ACTIVE` after that rejection. Manual deletion through [Google Cloud project settings](https://console.cloud.google.com/iam-admin/settings?project=mons-link) is still required; verify the resulting `DELETE_REQUESTED` state and repeat the affected API checks afterward. Do not treat the completed repository cleanup as proof of project shutdown.
