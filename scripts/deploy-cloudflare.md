...
`mons-link-profile-game-projection` is the permanent rating and automatch projector Queue. Automatch mutations atomically persist `profileGameProjectionOutbox/automatch/{inviteId}` before enqueueing, and every Worker or Firebase projector serializes work through the same per-invite lock. The five-minute Worker schedule repairs malformed automatch markers, repairs rating completion markers, claims pending automatch markers by `lastQueuedAtMs`, and re-enqueues both task kinds. Monitor Queue consumption, pending marker age, and `profile_game_projection_*` logs, including `profile_game_projection_enqueue_failed` entries whose kind is `automatch-profile-game-projection`.
